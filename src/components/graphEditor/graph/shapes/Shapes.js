   ...
   */
	...
	ExtendedShape.prototype.paintForeground = function(c, x, y, w, h)
	{
		if (this.style != null)
		{
			if (!this.outline && this.style["double"] == 1)
			{
				...
				if (w > 0 && h > 0)
				{
					mxRectangleShape.prototype.paintBackground.apply(this, [c, x, y, w, h]);
				}
			}
	...
	if (typeof mxVertexHandler !== "undefined")
	{
		...
		var handleFactory = {
            ...
            (mxUtils.getValue(this.state.style, mxConstants.STYLE_HORIZONTAL, 1) == 1) ?	Math.round(Math.max(0, Math.min(bounds.height, pt.y - bounds.y))) : Math.round(Math.max(0, Math.min(bounds.width, pt.x - bounds.x)));
				})];
