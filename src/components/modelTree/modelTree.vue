...
export default {
	...
	methods: {
		...
		getMenuItems(data){
			const modelDefine = this.factory.modelDefinePool[data.modelDefineId];
			const items = modelDefine.allowedNestModelDefineIds
				.map(id => this.factory.modelDefinePool[id])
				.map(item => ({ value: item.id, label: item.typeName, modelDefine: item }));
			return items;
		},
		addNode(parentNode, menuItem){
			if (!menuItem.modelDefine.isRelation){
				...
				this.factory.createRelation({
					...
					displayName: menuItem.modelDefine.typeName + parentNode.name + "->" + "zqq",
					attrs: cloneDeep(menuItem.modelDefine.attrs),
